...
void Budget::addPurchaseEntry(std::string purName, int purDate, int purAmt, bool iniPurchaseEntry){
        Purchase myPurch(purName, purDate, purAmt);
        purchases.push_back(myPurch);
        if(iniPurchaseEntry){
                amtSpent += purAmt;
                subtractRemainingAllowance(purAmt);
        }
}

void Budget::subtractRemainingAllowance(int amtSpent) {
        allowance -= amtSpent;
        std::cerr<<"SUBTRACT CALLED"<<std::endl;
}

void Budget::addRemainingAllowance(int amtGained) {
        allowance += amtGained;
}
...
void displayBudgets(std::vector<Budget> listOfBudgets, int budgetItr) {
        ...
        std::cout << "Remaining Allowance: " << listOfBudgets[budgetItr].getAllowance() << std::endl;
}
                ...
                for (int i = 0; i < lines.size(); i++) {
                        if (lines[i] == "-.-.-.-") {
                                ...
                                while ((lines[j] != "-.-.-.-") && (lines[j] != "***")) {
                                        myBudget.addPurchaseEntry(lines[j - 2], stringToInt(lines[j - 1]), stringToInt(lines[j]), false);
                                        j -= 3;
                                }
